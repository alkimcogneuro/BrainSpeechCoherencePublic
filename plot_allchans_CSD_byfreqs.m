function [] = plot_allchans_CSD_byfreqs(coherence_struct)    
    % coherence_struct is a structure containing the results of the cross spectral density analysis for one subject and one condition.
    ...
    % coherence_struct.CSD is num_trials x num_channels x num_freqs, 
    % where each element is the CSD value for that trial, channel, and frequency.
    ...
    csd_avg_mags = squeeze(mean(abs(coherence_struct.CSD), [1 2])); 
    size(csd_avg_mags)
    ...
    csd_avg_channel_means = abs(mean(coherence_struct.CSD_chanmeans, 1));  % average across channels (dimension 1)    
    ...
    csd_upperbd_freq = 7;  % upper bound of frequency range of interest

    idx = (coherence_struct.Freqs >= csd_lowerbd_freq) & (coherence_struct.Freqs <= csd_upperbd_freq);  % logical index for frequencies between 2 and 5 Hz

    % average across frequencies (dimension 2), resulting in a vector of length num_channels.
    csd_avg_2_5Hz_by_channel = mean(abs(coherence_struct.CSD_chanmeans(:, idx)), 2);  
    ...
    channel_locations = coherence_struct.Chanlocs; 
    ...
    %% plot_topomap(csd_avg_2_5Hz_by_channel', coherence_struct.Chanlocs)  % plot the topomap using the random voltages and the channel locations from the EEG data file.
    call_topoplot_eeglab(csd_avg_2_5Hz_by_channel', coherence_struct.Chanlocs,'CSD', 'CSD')  % plot the topomap using the random voltages and the channel locations from the EEG data file.
    for idx = 1:length(coherence_struct.Chanlocs)
        fprintf('channel %s, CSD(2-5Hz) = %d\n',  coherence_struct.Chanlocs(idx).labels,  csd_avg_2_5Hz_by_channel(idx) );
    end
    ...
    msc_avg_2_5Hz_by_channel = mean(abs(coherence_struct.MSC_chanmeans(:, idx)), 2);  % average across frequencies (dimension 2), resulting in a vector of length num_channels.
    % we need to convert to double precision for plotting, 
    % I think this is because the plotting code assumes that the input values can be very small, 
    % and when we plot them, they will be rounded to zero if we keep them in single precision.
    ...
    call_topoplot_eeglab(msc_avg_2_5Hz_by_channel', coherence_struct.Chanlocs,'Coherence', 'MSC')  % plot the topomap using the random voltages and the channel locations from the EEG data file.
    
    for idx = 1:length(coherence_struct.Chanlocs)
        fprintf('channel %s, MSC(2-5Hz) = %d\n',  coherence_struct.Chanlocs(idx).labels,  msc_avg_2_5Hz_by_channel(idx) );
    end

    figure; 
    for channel = 1:length(coherence_struct.Chanlocs) 
        chanlabel = coherence_struct.Chanlocs(channel).labels;
        %%Results_CSD_Analysis.Chanlocs.labels
        csd_channel_means = coherence_struct.CSD_chanmeans(channel, :);
        csd_channel_mags = abs(csd_channel_means);
        freqs = coherence_struct.Freqs;
        ...
        subplot(ceil(length(coherence_struct.Chanlocs)/4), 4, channel);
    ...
    %     chanlabel = coherence_struct.Chanlocs(channel).labels;
    %     %%Results_CSD_Analysis.Chanlocs.labels
    %     msc_channel_means = coherence_struct.MSC_chanmeans(channel, :);
