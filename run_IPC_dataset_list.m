...
function [] = run_IPC_dataset_list(eeg_data_filenames, speech_files_path, highpass_cutoff, lowpass_cutoff, options)
    % This script runs the Instantaneous Phase Coherence analysis analysis for all data files in a list of filenames, using the specified filter parameters.
    % For each data file, it runs Apply2Dataset_InstaPhaseCoherence() to analyze the entrainment between the EEG data and the corresponding speech data,    
    % 
    arguments
    ...
    % Run Apply2Dataset_InstaPhaseCoherence() for each one, using the corresponding speech data. 
    ...
    for idx = 1:length(eeg_data_filenames)
        ...
        % --------------------------------------------------------------------------------------%
        % Each call to Apply2Datset_InstaPhaseCoherence returns a structure, which we'll call Results_IPC_Analysis.
        % Results_IPC_Analysis will contain the results of the Instantaneous Phase Coherence analysis for one subject and one condition.
        % The Results_IPC_Analysis data structure will include fields for meta data about the analysis parameters,
        ...
        % --------------------------------------------------------------------------------------%
        % We create a filename, ipc_output_file for saving the analysis results.  
        ...
        if isnan(options.StartTimeOffset) || isnan(options.EpochDuration)
            Results_IPC_Analysis = Apply2Dataset_InstaPhaseCoherence(EEG_struct, Speech_RawData, highpass_cutoff, lowpass_cutoff);
        else  
            Results_IPC_Analysis = Apply2Dataset_InstaPhaseCoherence(EEG_struct, Speech_RawData, highpass_cutoff, lowpass_cutoff, StartTimeOffset=options.StartTimeOffset, EpochDuration=options.EpochDuration); 
        end
